...
const HourlyForecast: React.FC = () => {
  ...
  useEffect(() => {
    const fetchForecast = async () => {
      ...
      setError(null);
      try {
        const res = await axios.get(`http://localhost:5000/api/daily_forecast?meter_code=${selectedMeter}`);
        let parsedData;
    ...
    datasets: [
      {
        ...
        borderColor: 'rgba(19, 88, 236, 0.9)',
        borderWidth: 1.5,
        tension: 0.3,
        fill: false,
        backgroundColor: 'rgba(19, 88, 236, 0.9)',
        // Make points visible
      ...
      },
      {
        ...
        borderColor: 'rgba(230, 16, 16, 0.9)',
        borderWidth: 1.5,
        borderDash: [10, 5],
        tension: 0.3,
        fill: false,
  ...
  const chartOptions = useMemo(() => {
    return {
      ...
      plugins: {
        ...
        title: {
          display: true,
          text: `\n\nDaily Forecast for ${selectedMeter}`, // ✅ adds fake newlines ABOVE the legend to push chart down
          padding: {
      ...
      layout: {
        padding: {
          bottom: 30, // ✅ adds bottom padding to chart area
        }
      },
      scales: {
        x: {
