...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose }) => {
  ...
  const dashboardItems: SidebarItem[] = [
    ...
    // { path: "/dashboard/customer", icon: Users, label: "Customer Behavior" },
    // { path: "/dashboard/inventory", icon: Package, label: "Inventory Efficiency" },
    // { path: "/dashboard/operational", icon: Settings, label: "Operational Efficiency" },
    // { path: "/dashboard/project", icon: BarChart3, label: "Project Progress" },
    // { path: "/dashboard/financial", icon: DollarSign, label: "Financial Health" },
  ];
          ...
          <div className="mt-3 space-y-1">
            {dashboardItems.map((item) => (
              <NavLink
                key={item.path}
                to={item.path}
                className={({ isActive }) => `sidebar-item flex items-center py-3 px-2 rounded-md ${isActive ? "active bg-blue-600 text-white dark:bg-blue-600" : ""}`}
                onClick={() => onClose()}
              >
